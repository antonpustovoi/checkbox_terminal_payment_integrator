  ...
  override fun definition() = ModuleDefinition {
    ...
    AsyncFunction("startActivity") {
      ...
      if (pendingPromise != null) {
        var exception = Throwable("IntentLauncher activity is already started").toCodedException()
        pendingPromise?.reject(exception)
        pendingPromise = null
      }
      ...
      params.extra?.let {
        val valuesList =
                it.mapValues { (_, value) -> if (value is Double) value.toInt() else value }
